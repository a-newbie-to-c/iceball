...
if compat.jit then
    ...
    function byteset_new (t)
        --   print ("Konstructor", type(t), t)
        if type(t) == "number" then
            local res = boolset_constructor(t+1)
            res.upper = t
            --  for i = 0, res.upper do if res[i] then print("K", i, res[i]) end end
            return res
        end
        local upper = u_max(t)
...
function byteset_tostring (s)
    local list = {}
    for i = 0, 255 do
        --   print(s[i] == true and i)
        list[#list+1] = (s[i] == true) and i or nil
    end
    --   print("BS TOS", t_concat(list,", "))
