...
--  local debug, rawset, setmetatable, error, print, expose 
--      = debug, rawset, setmetatable, error, print, u.expose
...
function insert (caps, sbj, vals, ci, vi)
    ...
    --   print("Insert - kind = ", kind[ci])
    while kind[ci] and openclose[ci] >= 0 do
        --   print("Eval, Pre Insert, kind:", kind[ci], ci)
        ci, vi = eval[kind[ci]](caps, sbj, vals, ci, vi)
        --   print("Eval, Post Insert, kind:", kind[ci], ci)
    end
...
function lookback (caps, label, ci)
    --   print("lookback( "..tostring(label).." ), ci = "..ci) --.." ..."); --expose(caps)
    --   if ci == 9 then error() end
    local aux, openclose, kind= caps.aux, caps.openclose, caps.kind

    repeat
        --   print("Lookback kind: ", kind[ci], ", ci = "..ci, "oc[ci] = ", openclose[ci], "aux[ci] = ", aux[ci])
        ci = ci - 1
...
function eval.Cb (caps, sbj, vals, ci, vi)
    ...
    --   print(" Eval Cb, Cb_ci = "..Cb_ci)
    Cb_ci, vi = eval.Cg(caps, sbj, vals, Cb_ci, vi)
    --   print("/Eval Cb next kind, ", caps.kind[ci + 1], "Values = ..."); expose(vals)
...
function eval.Cc (caps, sbj, vals, ci, vi)
    ...
    --   print"Eval Cc"; expose(these_values)
...
function eval.Cg (caps, sbj, vals, ci, vi)
    --   print("Gc - caps", ci, caps.openclose[ci]) expose(caps)
    if caps.openclose[ci] > 0 then
        --   print("Cg - closed")
        vals[vi] = s_sub(sbj, caps.bounds[ci], caps.openclose[ci] - 1)
        return ci + 1, vi + 1
...
function eval.Cs (caps, sbj, vals, ci, vi)
    ...
    if caps.openclose[ci] > 0 then
        ...
        local last
        ci = ci + 1
        --   print"eval.CS, openclose: "; expose(openclose)
        --   print("eval.CS, ci =", ci)
        while openclose[ci] >= 0 do
            --   print(" eval Cs - ci = "..ci..", bi = "..bi.." - LOOP - Buffer = ...")
            --   u.expose(buffer)
            --   print(" eval - Cs kind = "..kind[ci])
            ...
            --   print("  Cs post eval ci = "..ci..", Cs_vi = "..Cs_vi)
            if Cs_vi > 1 then
                buffer[bi] = Cs_vals[1]
                bi = bi + 1
...
local function div_str_cap_refs (caps, ci)
    ...
    repeat
        local oc = opcl[ci]
        --   print("/''refs", caps.kind[ci], ci, oc, depth)
...
function eval.div_string (caps, sbj, vals, ci, vi)
    ...
    --   print("  REFS div_string ci = "..ci..", n = ", n, ", refs = ...")
    --   expose(refs)
...
function LL.evaluate (caps, sbj, ci)
    --   print("*** Eval", caps, sbj, ci)
    --   expose(caps)
    --   cprint(caps, sbj, ci)
    local vals = {}
    --   vals = setmetatable({}, {__newindex = function(self, k,v) 
    --       print("set Val, ", k, v, debug.traceback(1)) rawset(self, k, v) 
    --   end})
