...
function clear_captures(ary, ci)
    --   print("clear caps, ci = ", ci)
    --   print("TRACE: ", debug.traceback(1))
    --   expose(ary)
...
--  local dbgcapsmt = {__newindex = function(self, k,v) 
--      if k ~= #self + 1 then 
--          print("Bad new cap", k, v)
--          expose(self)
--          error""
--      else
--          rawset(self,k,v)
...
--  local
--  function dbgcaps(t) return setmetatable(t, dbgcapsmt) end
...
for _, v in pairs{ 
    ...
    return function (pt, ccache)
        --   print("Compiling", "XXXX")
        --   expose(LL.getdirect(pt))
        --   LL.pprint(pt)
        local matcher, this_aux = compile(pt.pattern, ccache), pt.aux
        return function (sbj, si, caps, ci, state)
            --   print("XXXX: ci = ", ci, "             ", "", ", si = ", si, ", type(this_aux) = ", type(this_aux), this_aux)
            --   expose(caps)
            ...
            if success then
                --   print("/XXXX: ci = ", ci, ", ref_ci = ", ref_ci, ", si = ", si)
                if ci == ref_ci + 1 then
                    --   print("full", si)
...
for _, v in pairs{ 
    ...
    return function (pt, ccache)
        ...
        return function (sbj, si, caps, ci, state)
            --   print("XXXX: ci = ", ci, ", aux = ", this_aux, ", si = ", si)
...
compilers["Cmt"] = function (pt, ccache)
    ...
    return function (sbj, si, caps, ci, state)
        ...
        if not success then 
            --   print("/Cmt No match", ".....",  (" -"..mt.."- "..n):rep(12))
            --   n = n - 1
            clear_captures(caps.aux, ci)
            --   expose(caps)
        ...
        --   print("Cmt match! ci = ", ci, ", Cmt_ci = ", Cmt_ci)
        --   expose(caps)
        ...
        --   print("Cmt values ..."); expose(values)
        --   print("Cmt, final_si = ", final_si, ", Cmt_si = ", Cmt_si)
        --   print("SOURCE\n",sbj:sub(Cmt_si-20, Cmt_si+20),"\n/SOURCE")
        if not final_si then 
            --   print("/Cmt No return", ".....",  (" +"..mt.."- "..n):rep(12))
            --   n = n - 1
            -- clear_captures(caps.aux, ci)
            --   expose(caps)
            return false, si, ci
        ...
        and final_si <= #sbj + 1 
        then
            --   print("Cmt Success", values, values and values.n, ci)
            local kind, bounds, openclose, aux 
                = caps.kind, caps.bounds, caps.openclose, caps.aux
            for i = 1, values.n do
                kind      [ci] = "value"
        ...
        elseif type(final_si) == "number" then
            ...
            --   print("/Cmt success - si = ", si,  ", ci = ", ci, ".....",  (" +"..mt.."+ "..n):rep(8))
            --   n = n - 1
            --   expose(caps)
...
compilers["string"] = function (pt, ccache)
    ...
    local N = #S
    return function(sbj, si, caps, ci, state)
         --   print("String    ",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
        local in_1 = si - 1
        for i = 1, N do
            local c
            c = s_byte(sbj,in_1 + i)
            if c ~= S[i] then
         --   print("%FString    ",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
                return false, si, ci
            end
        end
         --   print("%SString    ",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
        return true, si + N, ci
    end
end
...
compilers["char"] = function (pt, ccache)
    return load(([=[
        local s_byte, s_char = ...
        return function(sbj, si, caps, ci, state)
            --   print("Char "..s_char(__C0__).." ", caps.kind[ci - 1], ", si = "..si, ", ci = "..ci, sbj:sub(1, si - 1))
...
function truecompiled (sbj, si, caps, ci, state)
     --   print("True    ",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
    return true, si, ci
end
...
function falsecompiled (sbj, si, caps, ci, state)
     --   print("False   ",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
    return false, si, ci
end
...
function eoscompiled (sbj, si, caps, ci, state)
     --   print("EOS     ",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
    return si > #sbj, si, ci
end
...
function onecompiled (sbj, si, caps, ci, state)
    --   print("One", caps.kind[ci - 1], ", si = "..si, ", ci = "..ci, sbj:sub(1, si - 1))
...
compilers["any"] = function (pt)
    ...
    if N == 1 then
        ...
        N = pt.aux - 1
        return function (sbj, si, caps, ci, state)
            --   print("Any", caps.kind[ci - 1], ", si = "..si, ", ci = "..ci, sbj:sub(1, si - 1))
            local n = si + N
            if n <= #sbj then
                --   print("/Any success", caps.kind[ci - 1], ", si = "..si, ", ci = "..ci, sbj:sub(1, si - 1))
                return true, n + 1, ci
            else
                --   print("/Any fail", caps.kind[ci - 1], ", si = "..si, ", ci = "..ci, sbj:sub(1, si - 1))
                return false, si, ci
            end
        end
    end
...
do
    ...
    compilers["grammar"] = function (pt, ccache)
        ...
        local start = gram[1]
        return function (sbj, si, caps, ci, state)
             --   print("Grammar ",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
...
compilers["behind"] = function (pt, ccache)
    ...
    return function (sbj, si, caps, ci, state)
         --   print("Behind  ",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
...
compilers["range"] = function (pt)
    local ranges = pt.aux
    return function (sbj, si, caps, ci, state)
         --   print("Range   ",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
...
compilers["set"] = function (pt)
    local s = pt.aux
    return function (sbj, si, caps, ci, state)
        --   print("Set, Set!, si = ",si, ", ci = ", ci)
        --   expose(s)
        local char, nsi = s_byte(sbj, si), si + 1
        --   print("Set, Set!, nsi = ",nsi, ", ci = ", ci, "char = ", char, ", success = ", (not not s[char]))
        if s[char]
...
compilers["ref"] = function (pt, ccache)
    ...
    local ref
    return function (sbj, si, caps, ci, state)
         --   print("Reference",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
        if not ref then
        ...
        --   print("Ref - <"..tostring(name)..">, si = ", si, ", ci = ", ci)
...
local choice_tpl = [=[
             --   print(" Choice XXXX, si = ", si, ", ci = ", ci)
            success, si, ci = XXXX(sbj, si, caps, ci, state)
             --   print(" /Choice XXXX, si = ", si, ", ci = ", ci, ", success = ", success)
            if success then
                return true, si, ci
            else
...
compilers["choice"] = function (pt, ccache)
    ...
    local compiled = t_concat{
        ...
        return function (sbj, si, caps, ci, state)
             --   print("Choice ", ", si = "..si, ", ci = "..ci, sbj:sub(1, si-1)) --, sbj)
...
local sequence_tpl = [=[
            --   print(" Seq XXXX , si = ",si, ", ci = ", ci)
            success, si, ci = XXXX(sbj, si, caps, ci, state)
            --   print(" /Seq XXXX , si = ",si, ", ci = ", ci, ", success = ", success)
            if not success then
    ...
    local compiled = t_concat{
        ...
        return function (sbj, si, caps, ci, state)
            local ref_si, ref_ci, success = si, ci
             --   print("Sequence ", ", si = "..si, ", ci = "..ci, sbj:sub(1, si-1)) --, sbj)
            ]=],
            t_concat(chunks,"\n"),[=[
             --   print("/Sequence ", ", si = "..si, ", ci = "..ci, sbj:sub(1, si-1)) --, sbj)
            return true, si, ci
...
compilers["at most"] = function (pt, ccache)
    ...
    n = -n
    return function (sbj, si, caps, ci, state)
         --   print("At most   ",caps, caps and caps.kind or "'nil'", si) --, sbj)
        local success = true
        for i = 1, n do
...
compilers["at least"] = function (pt, ccache)
    ...
    if n == 0 then
        return function (sbj, si, caps, ci, state)
            --   print("Rep  0", caps.kind[ci - 1], ", si = "..si, ", ci = "..ci, sbj:sub(1, si - 1))
    ...
    elseif n == 1 then
        return function (sbj, si, caps, ci, state)
            --   print("At least 1 ",caps, caps and caps.kind or "'nil'", ci, si, state) --, sbj)
            local last_si, last_ci
            local success = true
            --   print("Rep  1", caps.kind[ci - 1], ", si = "..si, ", ci = "..ci)
            success, si, ci = matcher(sbj, si, caps, ci, state)
            if not success then
            --   print("/Rep  1 Fail")
                -- clear_captures(caps.aux, ci)
            ...
            while true do
                local success
                --   print(" rep  1", caps.kind[ci - 1], ", si = "..si, ", ci = "..ci, sbj:sub(1, si - 1))
                --   N=N+1
                last_si, last_ci = si, ci
                success, si, ci = matcher(sbj, si, caps, ci, state)
                if not success then                     
        ...
        return function (sbj, si, caps, ci, state)
            --   print("At least "..n.." ", caps and caps.kind or "'nil'", ci, si, state) --, sbj)
            local last_si, last_ci
            local success = true
            for _ = 1, n do
                success, si, ci = matcher(sbj, si, caps, ci, state)
            ...
            while true do
                local success
                --   print(" rep  "..n, caps.kind[ci - 1], ", si = "..si, ", ci = "..ci, sbj:sub(1, si - 1))
                last_si, last_ci = si, ci
                success, si, ci = matcher(sbj, si, caps, ci, state)
                if not success then                     
                    si, ci = last_si, last_ci
...
compilers["unm"] = function (pt, ccache)
    ...
    local matcher = compile(pt.pattern, ccache)
    return function (sbj, si, caps, ci, state)
         --   print("Unm     ", caps, caps and caps.kind or "'nil'", ci, si, state)
        -- Throw captures away
...
compilers["lookahead"] = function (pt, ccache)
    local matcher = compile(pt.pattern, ccache)
    return function (sbj, si, caps, ci, state)
        --   print("Look ", caps.kind[ci - 1], ", si = "..si, ", ci = "..ci, sbj:sub(1, si - 1))
        -- Throw captures away
        local success, _, _ = matcher(sbj, si, caps, ci, state)
         --   print("Look, success = ", success, sbj:sub(1, si - 1))
         -- clear_captures(caps.aux, ci)
        return success, si, ci
    end
end
